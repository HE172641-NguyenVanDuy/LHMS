        ...
        if (service == null) {
            ...
            if (service.equals("insertBill")) {
                ...
                for (ServiceOfRoom serviceOfRoom : listOfSR) {
                    if (serviceOfRoom.getServiceOfLodgingHouse().getServiceId() == 1) {
                        priceElectronic = serviceOfRoom.getPrice() * (newElectronic - oldElectronic);
                    }

                    if (serviceOfRoom.getServiceOfLodgingHouse().getServiceId() == 2) {
                        priceWater = serviceOfRoom.getPrice() * (newWater - oldWater);
                    }

                    if (serviceOfRoom.getServiceOfLodgingHouse().getServiceId() == 3 || serviceOfRoom.getServiceOfLodgingHouse().getServiceId() == 4 || serviceOfRoom.getServiceOfLodgingHouse().getServiceId() == 5) {
