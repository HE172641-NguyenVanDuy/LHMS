...
public class RoomDAO extends DBContext {
    ...
    public Room getRoomByAccountId(int accountId) {
        String sql = "SELECT r.* FROM Account a JOIN AccountInRoom ar\n"
                + "ON a.AccountId = ar.AccountId\n"
                + "JOIN Rooms r\n"
                + "ON r.RoomId = ar.RoomId WHERE a.AccountId = ?";
        LodgingHousesDAO lodgingHousesDAO = new LodgingHousesDAO();
        try {
            ...
            if(rs.next()) {
                ...
                room.setMaxOfQuantity(rs.getInt(3));
                room.setImage(rs.getString(4));
                room.setDescription(rs.getString(5));
                room.setLodgingHouse(lodgingHousesDAO.getLodgingHouseById(rs.getInt(6)));
                room.setStatusDelete(rs.getInt(7));
                room.setRoomName(rs.getString(8));
                room.setStatus(rs.getInt(9));
                return room;
            }
        } catch (Exception e) {
            e.printStackTrace();
